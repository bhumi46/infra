...
| `testrigs-dsf.yaml` | API test rig, UI test rig, DSL test rig. For `esignet` profile: deploys apitestrig into esignet/`${MOSIPID1_NS}`/`${MOSIPID2_NS}`/esignet-sunbird namespaces + optional signup apitestrig and signup uitestrig (disabled by default) |
...
| -16 | postgres-init-esignet (`0.0.1-develop`), redis | esignet / redis | Dynamic DB creation — one DB per esignet namespace (esignet, `${MOSIPID1_NS}`, `${MOSIPID2_NS}`, esignet-sunbird) |
...
| -11 | esignet-keycloak-init | keycloak | Runs in **keycloak ns** — chart creates `keycloak-host` CM + `keycloak-client-secrets` secret there (all keycloak resources already present). postInstall fans both out to esignet, `${MOSIPID1_NS}`, `${MOSIPID2_NS}`, esignet-sunbird. `wait: true` so postInstall only fires after job completes. |
| -10 | captcha | captcha |
| -9 | minio | minio |
...
| -16 | esignet-softhsm-mosipid1 | `${MOSIPID1_NS}` | Plugin 2 (mosip-identity); hook: `env ESIGNET_NS=${MOSIPID1_NS} softhsm-esignet-setup.sh` |
| -16 | esignet-softhsm-mosipid2 | `${MOSIPID2_NS}` | Plugin 2 (mosip-identity); same generic hook with MOSIPID2_NS |
...
| -14 | esignet-mosipid1 | `${MOSIPID1_NS}` | Plugin 2 (mosip-identity); DSF overrides `extraEnvVarsCM[0]: esignet-softhsm-mosipid1-share`, `extraEnvVarsCM[1]: esignet-global`; hook: `env ESIGNET_NS=... MOSIPID_SUFFIX=mosipid1 ... esignet-mosipid-preinstall.sh` |
| -14 | esignet-mosipid2 | `${MOSIPID2_NS}` | Plugin 2 (mosip-identity); same pattern with MOSIPID2 vars |
| -14 | esignet-sunbird | esignet-sunbird | Plugin 3 (sunbird-rc) |
| -13 | oidc-ui | esignet | Istio host: `esignet.${domain_name}` |
| -13 | oidc-ui-mosipid1 | `${MOSIPID1_NS}` | Istio host: `esignet-mosipid1.${domain_name}`; hook: `env ESIGNET_NS=${MOSIPID1_NS} oidc-ui-preinstall.sh` |
| -13 | oidc-ui-mosipid2 | `${MOSIPID2_NS}` | Istio host: `esignet-mosipid2.${domain_name}` |
...
| -10 | mock-relying-party-service-mosipid1 | `${MOSIPID1_NS}` | Istio via `healthservices-mosipid1.${domain_name}`; hook: `env ESIGNET_NS=${MOSIPID1_NS} mock-relying-party-service-preinstall.sh` |
| -10 | mock-relying-party-service-mosipid2 | `${MOSIPID2_NS}` | Istio via `healthservices-mosipid2.${domain_name}` |
...
| -9 | mock-relying-party-ui-mosipid1 | `${MOSIPID1_NS}` | hook: `env ESIGNET_NS=${MOSIPID1_NS} mock-relying-party-ui-mosipid-preinstall.sh` |
| -9 | mock-relying-party-ui-mosipid2 | `${MOSIPID2_NS}` | same generic hook with MOSIPID2_NS |
| -9 | mock-relying-party-ui-sunbird | esignet-sunbird | |
| -9 | pms-partner-mosipid1 | `${MOSIPID1_NS}` | PMS partner; preInstall creates `pms-partner-mosipid1-gateway` (chart has no gateway template); DSF overrides `extraEnvVarsCM[0]: esignet-global`, `extraEnvVarsCM[1]: esignet-config-server-share` |
| -9 | pms-policy-mosipid1 | `${MOSIPID1_NS}` | PMS policy; VS references `pms-partner-mosipid1-gateway` (shared with pms-partner); same `extraEnvVarsCM` overrides |
| -9 | pms-partner-mosipid2 | `${MOSIPID2_NS}` | PMS partner; preInstall creates `pms-partner-mosipid2-gateway`; hook: `env ESIGNET_NS=${MOSIPID2_NS} MOSIPID_SUFFIX=mosipid2 pms-partner-mosipid-preinstall.sh` |
| -9 | pms-policy-mosipid2 | `${MOSIPID2_NS}` | PMS policy; VS references `pms-partner-mosipid2-gateway` |
...
| `${MOSIPID1_DOMAIN}` | Workflow input `mosipid1_domain_name` or `vars.MOSIPID1_DOMAIN` | eSignet standalone — connecting MOSIP env domain for mosipid1 (IDA service URLs, Keycloak IAM host, DB server in testrigs) |
| `${MOSIPID2_DOMAIN}` | Workflow input `mosipid2_domain_name` or `vars.MOSIPID2_DOMAIN` | eSignet standalone — connecting MOSIP env domain for mosipid2 |
| `${SUNBIRD_DOMAIN}` | Workflow input or `vars.SUNBIRD_DOMAIN` | eSignet standalone — Sunbird registry domain (esignet-sunbird instance) |
| `${MOSIPID1_NS}` | Workflow input `mosipid1_ns` or `vars.MOSIPID1_NS` (default: `esignet-mosipid1`) | eSignet standalone — namespace for mosipid1 instance; used as YAML key in DSF (Helmsman substitutes before YAML parse) |
| `${MOSIPID2_NS}` | Workflow input `mosipid2_ns` or `vars.MOSIPID2_NS` (default: `esignet-mosipid2`) | eSignet standalone — namespace for mosipid2 instance |
...
| `esignet-init-db.sh` | postgres-init-esignet | Pre-creates all 4 esignet namespaces (`esignet`, `${MOSIPID1_NS}`, `${MOSIPID2_NS}`, `esignet-sunbird`) with Istio label at priority -16 so postInstall can copy `db-common-secrets` into each |
...
| `config-server-esignet-setup.sh` | esignet-config-server | Creates esignet ns + Istio label; copies db secrets (`db-common-secrets` from postgres ns, `redis`/`redis-config` from redis ns); creates `esignet-global` CM in esignet ns with 10 keys from `${domain_name}`: `installation-domain`, `mosip-api-host`, `mosip-api-internal-host`, `mosip-esignet-host` (`esignet.${domain_name}`), `mosip-iam-external-host`, `mosip-kafka-host`, `mosip-postgres-host`, `mosip-signup-host`, `mosip-smtp-host`, `mosip-version`; pre-creates empty `esignet-misp-onboarder-key` placeholder. **Does NOT copy softhsm secret** — the softhsm Helm chart installs directly into the esignet namespace. **Does NOT copy keycloak resources** — those are pre-populated by `esignet-postinstall-keycloak-init.sh` in external-dsf |
| `config-server-esignet-postinstall.sh` | esignet-config-server | Copies `esignet-config-server-share` CM from `esignet` ns to `${MOSIPID1_NS}`, `${MOSIPID2_NS}`, `esignet-sunbird` so those instances can locate the config-server |
| `esignet-preinstall-keycloak-init.sh` | esignet-keycloak-init | Deletes old `esignet-keycloak-init` release from keycloak ns (helm manages `keycloak-host` and `keycloak-client-secrets` — no manual kubectl delete needed); fetches all 5 client secrets from keycloak ns for Helmsman `${VAR}` substitution (empty on fresh install — chart generates them) |
| `esignet-postinstall-keycloak-init.sh` | esignet-keycloak-init | Fans out `keycloak-host` CM, `keycloak-env-vars` CM, `keycloak` secret, and `keycloak-client-secrets` secret from keycloak ns to all 4 esignet namespaces (esignet, `${MOSIPID1_NS}`, `${MOSIPID2_NS}`, esignet-sunbird); skips any namespace that does not exist |
| `softhsm-esignet-setup.sh` / `softhsm-esignet-postinstall.sh` | softhsm-esignet | HSM namespace + secret sharing (esignet ns) |
| (generic via DSF `env ESIGNET_NS=... softhsm-esignet-setup.sh`) | softhsm-esignet-mosipid1 / softhsm-esignet-mosipid2 / softhsm-esignet-sunbird | No separate wrapper script — DSF injects `ESIGNET_NS` via `env` prefix so one script handles all namespaces |
| `esignet-preinstall.sh` | esignet | Copies postgres/redis configmaps+secrets to esignet ns |
| `esignet-mosipid-preinstall.sh` | esignet-mosipid1 / esignet-mosipid2 | **Generic script** invoked via `env ESIGNET_NS=... MOSIPID_DOMAIN=... MOSIPID_SUFFIX=mosipid1\|mosipid2 MOSIPID_CAPTCHA_SITE_KEY=... MOSIPID_CAPTCHA_SECRET_KEY=... MOSIPID_POSTGRES_PASSWORD=... MOSIPID_KEYCLOAK_ADMIN_PASSWORD=...`. Calls base `esignet-preinstall.sh`, then: (1) creates namespace-specific `esignet-global` CM with `mosip-esignet-host: esignet-mosipid1\|2.${domain_name}`; (2) patches `postgres-config` with `mosip_esignet_${MOSIPID_SUFFIX}` / `esignetuser_${MOSIPID_SUFFIX}`; (3) creates MISP onboarder key placeholder; (4) creates `esignet-captcha-${MOSIPID_SUFFIX}` in captcha ns + patches captcha deployment with `MOSIP_CAPTCHA_GOOGLERECAPTCHAV2_SECRET_ESIGNET${UPPER_SUFFIX}`; (5) creates `postgres-postgresql-${MOSIPID_SUFFIX}` secret; (6) creates `keycloak-host-${MOSIPID_SUFFIX}` CM with `iam.${MOSIPID_DOMAIN}`; (7) fetches all confidential clients from remote Keycloak REST API → creates `keycloak-client-secrets-${MOSIPID_SUFFIX}` |
| `oidc-ui-preinstall.sh` | oidc-ui (all namespaces) | Waits for esignet pods ready; reads `ESIGNET_NS` (defaults to `esignet`) — used for both main and mosipid instances via DSF `env ESIGNET_NS=...` prefix |
| `mock-identity-system-preinstall.sh` | mock-identity-system | Copies `softhsm-mock-identity-system` secret from softhsm ns; creates `mockid-postgres-config` with defaults for `database-name`/`database-username`/`database-port` (`mosip_mockidentitysystem`/`mockidsystemuser`/`5432`) — overridable via `MOCKID_DB_NAME`/`MOCKID_DB_USER`/`MOCKID_DB_PORT`; verifies `softhsm-mock-identity-system-share` CM is present |
| `mock-relying-party-service-preinstall.sh` | mock-relying-party-service (all namespaces) | Creates K8s secrets `mock-relying-party-private-key-jwk` (key: `client-private-key`) and `jwe-userinfo-service-secrets` from `$MOCK_RELYING_PARTY_CLIENT_PRIVATE_KEY` and `$MOCK_RELYING_PARTY_JWE_PRIVATE_KEY`; reads `ESIGNET_NS` (defaults to `esignet`) — used for mosipid1/mosipid2/sunbird via DSF `env ESIGNET_NS=...` prefix |
| `mock-relying-party-ui-mosipid-preinstall.sh` | mock-relying-party-ui-mosipid1 / mosipid2 | **Generic script** — reads `ESIGNET_NS`; creates namespace, checks for service existence; replaces old per-namespace wrapper scripts |
| `mock-relying-party-ui-preinstall.sh` | mock-relying-party-ui | Namespace creation + service check (esignet ns) |
| `esignet-misp-onboarder-preinstall.sh` | esignet-misp-onboarder | Disables Istio injection; deletes stale `esignet-onboarder-config` CM + `esignet-onboarder-secrets` secret; copies `keycloak-env-vars`/`keycloak`/`keycloak-client-secrets` from keycloak ns; builds `s3` secret from MinIO root-password; deletes `onboarder-namespace` CM; waits for esignet ready |
...
| `esignet-mock-rp-onboarder-preinstall.sh` | esignet-mock-rp-onboarder | Disables Istio injection; copies `keycloak-env-vars` configmap + `keycloak`/`keycloak-client-secrets` secrets from keycloak ns; builds `s3` secret from MinIO root-password; deletes `onboarder-namespace` CM before install; waits for esignet ready |
...
| `pms-partner-mosipid-preinstall.sh` | pms-partner-mosipid1 / pms-partner-mosipid2 | **Generic script** — reads `ESIGNET_NS` and `MOSIPID_SUFFIX`; creates `pms-partner-${MOSIPID_SUFFIX}-gateway` (HTTPS+HTTP, TLS credential `pms-partner-${MOSIPID_SUFFIX}-tls`); chart has no gateway template |
...
| `apitestrig-esignet-mosipid-setup.sh` | esignet-mosipid1-apitestrig / esignet-mosipid2-apitestrig | **Generic script** — reads `ESIGNET_NS`; same cleanup+copy logic as esignet-apitestrig. Invoked via DSF `env ESIGNET_NS=${MOSIPID1_NS}` / `env ESIGNET_NS=${MOSIPID2_NS}` |
...
| `trigger-test-jobs-esignet.sh` | esignet-sunbird-apitestrig (postInstall) | Triggers all cronjobs in esignet, `${MOSIPID1_NS}`, `${MOSIPID2_NS}`, esignet-sunbird namespaces sequentially; optionally triggers signup and signup-uitestrig if deployed. Uses `trigger_all_in_ns` so cronjob names are auto-detected |
...
| `softhsm-esignet-mosipid1-values.yaml` | SoftHSM for `${MOSIPID1_NS}` — independent PVC and pin secret |
| `softhsm-esignet-mosipid2-values.yaml` | SoftHSM for `${MOSIPID2_NS}` — independent PVC and pin secret |
| `softhsm-esignet-sunbird-values.yaml` | SoftHSM for esignet-sunbird — independent PVC and pin secret |
| `softhsm-mock-identity-system-values.yaml` | SoftHSM for mock identity |
| `esignet-plugin-values.yaml` | Plugin 1 (mock) — captcha `secretKeyRef` active + `MOSIP_ESIGNET_CAPTCHA_REQUIRED=false`; used by `esignet` (main ns) |
| `esignet-mosipid1-plugin-values.yaml` | Plugin 2 (mosip-identity) — overrides full `extraEnvVars` list (SOFTHSM_ESIGNET_SECURITY_PIN references `esignet-softhsm-mosipid1`); captcha references `esignet-captcha-mosipid1`; used by `esignet-mosipid1` |
| `esignet-mosipid2-plugin-values.yaml` | Plugin 2 (mosip-identity) — identical pattern; SOFTHSM_ESIGNET_SECURITY_PIN references `esignet-softhsm-mosipid2`; used by `esignet-mosipid2` |
| `esignet-sunbird-plugin-values.yaml` | Plugin 3 (sunbird-rc) — captcha + sunbird registry URL + `NoOpKeyBinder`; SOFTHSM_ESIGNET_SECURITY_PIN references `esignet-softhsm-sunbird`; used by `esignet-sunbird` |
...
| `ESIGNET_MOSIPID1_CAPTCHA_SITE_KEY` | Plain text | captcha secret for `${MOSIPID1_NS}` (esignet standalone only) |
| `ESIGNET_MOSIPID1_CAPTCHA_SECRET_KEY` | Plain text | captcha secret for `${MOSIPID1_NS}` (esignet standalone only) |
| `ESIGNET_MOSIPID2_CAPTCHA_SITE_KEY` | Plain text | captcha secret for `${MOSIPID2_NS}` (esignet standalone only) |
| `ESIGNET_MOSIPID2_CAPTCHA_SECRET_KEY` | Plain text | captcha secret for `${MOSIPID2_NS}` (esignet standalone only) |
...
| `MOSIPID1_KEYCLOAK_ADMIN_PASSWORD` | Plain text | Keycloak admin password for MOSIP-ID instance 1 — used by `esignet-mosipid-preinstall.sh` to fetch all client secrets via Keycloak REST API into `keycloak-client-secrets-mosipid1` |
| `MOSIPID2_KEYCLOAK_ADMIN_PASSWORD` | Plain text | Keycloak admin password for MOSIP-ID instance 2 — creates `keycloak-client-secrets-mosipid2` |
| `MOSIPID1_POSTGRES_PASSWORD` | Plain text | Postgres superuser password for MOSIP-ID instance 1 — creates `postgres-postgresql-mosipid1` in `${MOSIPID1_NS}` |
| `MOSIPID2_POSTGRES_PASSWORD` | Plain text | Postgres superuser password for MOSIP-ID instance 2 — creates `postgres-postgresql-mosipid2` in `${MOSIPID2_NS}` |
...
- Each namespace has its own `valuesFile:` with the appropriate plugin's `extraEnvVarsAdditional` entries active:
  ...
  - `esignet-mosipid1` → `utils/esignet-mosipid1-plugin-values.yaml` (plugin 2, mosip-identity — IDA URLs active)
  - `esignet-mosipid2` → `utils/esignet-mosipid2-plugin-values.yaml` (plugin 2, mosip-identity — IDA URLs active)
...
- **SoftHSM secret name follows the Helm release name**: the softhsm chart creates a secret named `<release-name>` (e.g. release `esignet-softhsm-mosipid1` → secret `esignet-softhsm-mosipid1`). The esignet chart's default `extraEnvVars` references `esignet-softhsm` — which only exists in the main esignet namespace. For mosipid1/mosipid2/sunbird, the full `extraEnvVars` list is overridden in each plugin values file so `SOFTHSM_ESIGNET_SECURITY_PIN` references the correct release-named secret (`esignet-softhsm-mosipid1`, `esignet-softhsm-mosipid2`, `esignet-softhsm-sunbird`). Do NOT use `fullnameOverride` in softhsm values files — it renames the share ConfigMap too, breaking the name convention for all downstream consumers
- **postgres-config is copied with esignet default DB values then patched per namespace**: `esignet-preinstall.sh` copies `postgres-config` from the postgres ns (which carries `mosip_esignet`/`esignetuser` values). The generic `esignet-mosipid-preinstall.sh` then runs `kubectl patch configmap postgres-config --type merge` to override `database-name` and `database-username` with `mosip_esignet_${MOSIPID_SUFFIX}`/`esignetuser_${MOSIPID_SUFFIX}` (e.g. `mosip_esignet_mosipid1`/`esignetuser_mosipid1`). The patch runs after the copy so only the two differing keys change, preserving host/port
- **`global` CM replaced by `esignet-global` for mosipid1/mosipid2 and pms charts**: the esignet chart defaults include `extraEnvVarsCM: [global, config-server-share]` which causes `configmap "global" not found` in mosipid1/mosipid2 and pms-partner/policy namespaces. These are overridden in `esignet-dsf.yaml` via indexed `extraEnvVarsCM[N]` keys: `extraEnvVarsCM[1]: esignet-global` for esignet-mosipid1/mosipid2; `extraEnvVarsCM[0]: esignet-global` + `extraEnvVarsCM[1]: esignet-config-server-share` for pms-partner/policy. **Each namespace creates its own `esignet-global`** — `esignet-mosipid-preinstall.sh` runs `kubectl create configmap ... | kubectl apply -f -` directly in the target ns (idempotent). All keys use `${domain_name}`; only `mosip-esignet-host` and `mosip-signup-host` differ per namespace (e.g. `esignet-mosipid1.${domain_name}` vs `esignet-mosipid2.${domain_name}`). Do NOT copy from esignet ns — the main esignet CM has `esignet.${domain_name}` which is wrong for mosipid instances. `esignet-config-server-share` is still copied by `config-server-esignet-postinstall.sh`
...
- **`ESIGNET_AUD_URL` must match `MOSIP_ESIGNET_HOST`**: in `mock-relying-party-service-mosipid1/mosipid2/sunbird`, the `ESIGNET_AUD_URL` hostname must exactly match the public-facing `MOSIP_ESIGNET_HOST` set for that namespace. For mosipid1 it is `esignet-mosipid1.${domain_name}` and for mosipid2 it is `esignet-mosipid2.${domain_name}` — mismatched hostnames cause token audience validation failures.
- **PMS Gateway TLS secrets must exist before deploy**: `pms-partner-mosipid1-tls` and `pms-partner-mosipid2-tls` must be present in `istio-system` before the gateway preInstall hook runs — provision these via cert-manager or import manually beforehand
- **PMS policy has no gateway of its own**: `pms-policy-mosipid1` and `pms-policy-mosipid2` VS both reference the corresponding `pms-partner-*-gateway`. If pms-partner is disabled or its hook hasn't run, pms-policy traffic will fail
...
- **Dynamic DB per namespace**: `scriptsDir` is fixed and must match the repo folder (`mosip_esignet`, `mosip_mockidentitysystem`); `dbName` and `dbUser` are customized per namespace (e.g. `mosip_esignet_mosipid1` / `esignetuser_mosipid1`). Adding a new namespace requires a new `databases.*` entry in the DSF with a unique `dbName` and `dbUser`
- **New database entries MUST include both `su` block AND `defaultDb`**: `0.0.1-develop` chart accesses `$dbVal.su.secret.name` for every enabled entry AND uses `defaultDb` as the initial psql connection target. Entries that exist in chart defaults (e.g. `mosip_esignet`, `mosip_mockidentitysystem`) inherit these automatically; NEW entries (e.g. `mosip_esignet_mosipid1`, `mosip_esignet_mosipid2`, `mosip_esignet_sunbird`) must explicitly set all of: `databases.<key>.su.user: "postgres"`, `databases.<key>.su.secret.name: "postgres-postgresql"`, `databases.<key>.su.secret.key: "postgres-password"`, `databases.<key>.defaultDb: "postgres"`. Missing `su` block → `nil pointer evaluating interface {}.secret` at template render. Missing `defaultDb` → `deploy.sh` connects to the wrong database for schema setup, causing `DROP SCHEMA`/`CREATE SCHEMA` to run in an existing DB (corrupting it), and then DDL in the target DB fails with `permission denied to create "pg_catalog.client_detail"` because the schema was never created there.
- **postgres-init image must be new enough to support `DB_SCRIPTS_DIR`**: The `0.0.1-develop` chart sets `DB_SCRIPTS_DIR` in the ConfigMap to decouple the scripts folder from the DB name. If the deployed `mosipqa/postgres-init:develop` image is old (pre-`DB_SCRIPTS_DIR` support), the script falls back to using `MOSIP_DB_NAME` as the scripts directory — causing `cd: db_scripts/mosip_esignet_mosipid1: No such file or directory`. The DSF explicitly sets `image.repository: "mosipqa/postgres-init"`, `image.tag: "develop"`, `image.pullPolicy: "Always"` to force a fresh pull on every Job run. Confirm the image is the new version by checking the log: new image prints `"Successfully cloned"` (correct spelling) and `"Executing db_script for ... (scripts dir: ...)"` before the cd.
...
- **`--skip-releases` flag does not exist in Helmsman v3.17.1** — use `-exclude-target <release>` (one flag per release) to exclude specific apps. The softhsm skip logic in `helmsman_esignet.yml` builds the argument as `-exclude-target esignet-softhsm -exclude-target esignet-softhsm-mosipid1 ...` inside the loop (iterating `"esignet-softhsm:esignet"`, `"esignet-softhsm-mosipid1:${MOSIPID1_NS}"`, etc.) rather than a comma-separated list.
...
- **`helmsman_testrigs.yml` requires `MOSIPID1_DOMAIN` and `MOSIPID2_DOMAIN` for the `esignet` profile** — `testrigs-dsf.yaml` uses `${MOSIPID1_DOMAIN}` and `${MOSIPID2_DOMAIN}` for mosipid1/mosipid2 apitestrig `db-server` and `mosip_components_base_urls`. Set `vars.MOSIPID1_DOMAIN` and `vars.MOSIPID2_DOMAIN` in the GitHub Environment, or provide them as workflow inputs `mosipid1_domain_name` / `mosipid2_domain_name`.
- **`helmsman_esignet.yml` validates 4 extra secrets for the `esignet` profile** — `MOSIPID1_POSTGRES_PASSWORD`, `MOSIPID2_POSTGRES_PASSWORD`, `MOSIPID1_KEYCLOAK_ADMIN_PASSWORD`, `MOSIPID2_KEYCLOAK_ADMIN_PASSWORD` are checked in the "Validate required secrets" deploy step (inside `if [ "$PROFILE" = "esignet" ]`). These are all Environment secrets (not vars). The workflow fails fast before Helmsman runs if any are missing.
